...
void my_socketCli(int *fd_socket, int type, char *ip, int port, SA *my_addr)
{
  ...
  if (type == MY_TCP) {
      ASSERT((*fd_socket = socket(PF_INET, SOCK_STREAM, 0)) != -1, "TCP socket init fail\n");
  } else if (type == MY_UDP) {
      ASSERT((*fd_socket = socket(PF_INET, SOCK_DGRAM, 0)) != -1, "UDP socket init fail\n");
  }
...
void my_socketSer(int *fd_socket, int type, char *ip, int port)
{
  ...
  my_addr.sin_port = htons(port);
  my_addr.sin_addr.s_addr = inet_addr(ip);
  ASSERT(bind(*fd_socket, (struct sockaddr*)&my_addr, sizeof(my_addr)) != -1, "bind fail\n");
  return;
}
...
void my_connect(int fd_socket, pSA server_addr, socklen_t len)
{
  if (connect(fd_socket, server_addr, len) == -1) {
